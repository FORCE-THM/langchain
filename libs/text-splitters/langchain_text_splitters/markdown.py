...
class MarkdownHeaderTextSplitter:
    ...
    def split_text_lines(self, lines: [dict]) -> List[Document]:
        """Split markdown file
        Args:
            text: Markdown file"""
        ...
        # Content and metadata of the chunk currently being processed
        current_content: List[str] = []
        current_metadata: Dict[str, Union[str, int]] = {}
        # Keep track of the nested header structure
        header_stack: List[HeaderType] = []
        initial_metadata: Dict[str, Union[str, int]] = {}

        in_code_block = False
        opening_fence = ""
        start_page_number = None  # Initialize start_page_number

        for line_dict in lines:
            line = line_dict['content']
            page_number = line_dict['page_number']
            
            if start_page_number is None:
                start_page_number = page_number  # Set start_page_number at the beginning of a chunk
            ...
            for sep, name in self.headers_to_split_on:
                ...
                if stripped_line.startswith(sep) and (
                    ...
                    # only if current_content is not empty
                    if current_content:
                        current_metadata["page_number"] = start_page_number  # Use start_page_number
                        lines_with_metadata.append(
                            {
                                "content": "\n".join(current_content),
                                "metadata": current_metadata.copy(),
                        ...
                        current_content.clear()
                        start_page_number = page_number  # Update start_page_number for the next chunk
